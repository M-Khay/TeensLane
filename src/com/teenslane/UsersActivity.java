...
public class UsersActivity extends SexActivity
{
    ...
    protected void onResume()
    {
		LocalService.chatting = 0;
		super.onResume();
		if(SexActivity.filterReturn == true)
		{
			SexActivity.loadedUsers = new ArrayList<JSONObject>();
			
			SexActivity.filterReturn = false;
	    	SexActivity.executeTask(new getUserData(), "load user data");
		}
    }
	...
	public void setupButtons()
	{
		...
		title.setText(getString(R.string.users_title));
	...
	private void showUsers(JSONArray result, final int offset) throws JSONException 
	{
		...
		else
		{
			...
			for(int i=0; i<lastElement; i++)
			{
				...
				if(user != null)
				{
					...
					try
					{
						...
						{
							String userImageUrl = SexActivity.REST_SERVER_NAME + SexActivity.REST_IMAGE_SIZE_148 + "/" + user.getString("image");
			...
			if(result.length() > SexActivity.usersPerPage)
			{
				if(offset >= maxOffset)
				{
					Toast.makeText(getApplicationContext(), getString(R.string.maxoffset), Toast.LENGTH_LONG).show();
				}
				else
				{
					line = null;
		...
		try 
     	{
	        ...
	        nameValuePairs.add(new BasicNameValuePair("per_page", String.valueOf(SexActivity.usersPerPage)));
